                ...
                [this](std::shared_ptr<const ClassifiedImage> rawImage,
                    ...
                    auto split = std::partition(
                        std::begin(segments), std::end(segments), [image](const RansacGoalModel::GoalSegment& segment) {
                            return arma::dot(convert<double, 3>(image.horizon_normal),
                                             (segment.left + segment.right) / 2)
                                   > 0;
                        });

                    // Make an array of our partitions
                    std::array<std::vector<RansacGoalModel::GoalSegment>::iterator,
                               RansacGoalModel::REQUIRED_POINTS + 1>
                    ...
                    for (auto& result : models) {
                        ...
                        while ((imagePoint[0] < image.dimensions[0]) && (imagePoint[0] > 0)
                            ...
                            if (DEBUG_GOAL_RANSAC) {
                                debug.push_back(
                                    std::make_tuple(convert<int, 2>(imagePoint),
                    ...
                    for (auto it = goals->begin(); it != goals->end();) {
                        ...
                        bool goalsParallel = std::abs(arma::dot(arma::cross(cbr, ctr), arma::cross(cbl, ctl)))
                                             < MAXIMUM_ANGLE_BETWEEN_SIDES;
                    ...
                    for (auto it = goals->begin(); it != goals->end(); ++it) {
                        ...
                        Eigen::Vector2d left_Angles(
                            std::atan2(left[1], left[0]),
                        ...
                        if (std::min(quad.getBottomRight()[0], quad.getBottomLeft()[0]) > MEASUREMENT_LIMITS_LEFT
                            ...
                            Eigen::Matrix3d rGCc_cov = convert<double, 3, 3>(
                                arma::diagmat(VECTOR3_COVARIANCE
                                              % covariance_amplifier));  // arma::diagmat(arma::vec3{0.01,0.01,0.001})
