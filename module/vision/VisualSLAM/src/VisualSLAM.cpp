    ...
    VisualSLAM::VisualSLAM(std::unique_ptr<NUClear::Environment> environment) : Reactor(std::move(environment)) {
            ...
            if (imageLoading == 1) {
                if (curImage < nImages) {
                    ...
                    while ((vTimestamps.at(curImage) - loop_start_timestamp) < halfASecond) {
                        ...
                        if (!raw_image.data) {
                            std::cout << "Could not open or find the image at " << vstrImageFilenames[curImage]
                                      << std::endl;
                        }
                            ...
                            for (int i = 0; i < raw_image.rows; ++i) {
                                data.insert(
                                    data.end(), raw_image.ptr<uchar>(i), raw_image.ptr<uchar>(i) + raw_image.cols);
                            }
                        }
        ...
        on<Trigger<Image>, Single>().then([this](const Image& newImage) {
            ...
            if (newImage.camera_id == 1)  // Just using one of the cameras
            {
                ...
                curFrame++;
                // std::cout << "Received Image #" << curFrame << " from camera ID: " << newImage.camera_id <<
                // std::endl;
                ...
                // OpenCV won't let me create a Mat from a const source because the newly created Mat points to the
                // original
                // data, and it cannot guarantee it won't modify the data. I need to use const_cast to create the Mat,
                // so
                ...
                const cv::Mat im_const(newImage.dimensions[1],
                                       newImage.dimensions[0],
                                       CV_8UC1,
                                       const_cast<uint8_t*>(newImage.data.data()));
                ...
                if ((logFile.is_open() == true) && (logFile.good() == true)) {
                    logFile << frameStart.time_since_epoch().count() << " "
                            << NUClear::clock::now().time_since_epoch().count();
        ...
        // Loop Closing thread
        on<Trigger<LaunchLoopClosing>>().then([this]() { SLAM.launchLoopClosing(); });
    }
