...
def register(command):
    ...
    command.add_argument(
        ...
        metavar='config',
        choices=[
            '', 'new', 'n', 'update', 'u', 'overwrite', 'o', 'pull', 'p',
            'ignore', 'i'
        ],
        default='new',
    ...
    command.add_argument(
        '-t', '--toolchain', dest='toolchain', action='store_true')
...
def run(ip_addr, hostname, config, scripts, user, toolchain, **kwargs):
    ...
    if toolchain:
        ...
        call(['rsync', '-avzuPl', '--checksum', '-e ssh'] + libs +
             [target_dir + 'toolchain'])

        # Set rpath for all libs on the remote machine
        cprint(
            'Setting rpath for all toolchain libs to {0}'.format(target_dir +
                                                                 'toolchain'),
            'blue',
            attrs=['bold'])
        ...
        cprint(
            'Running {0} on {1}'.format(command, host), 'blue', attrs=['bold'])
    ...
    data_files = []
    if type(b.cmake_cache['NUCLEAR_MODULE_DATA_FILES']) == str:
        data_files.append(b.cmake_cache['NUCLEAR_MODULE_DATA_FILES'])
    else:
        data_files.extend(b.cmake_cache['NUCLEAR_MODULE_DATA_FILES'])

    if type(b.cmake_cache['NUCLEAR_UTILITY_DATA_FILES']) == str:
        data_files.append(b.cmake_cache['NUCLEAR_UTILITY_DATA_FILES'])
    else:
        data_files.extend(b.cmake_cache['NUCLEAR_UTILITY_DATA_FILES'])

    config_files = [os.path.relpath(c, build_dir) for c in data_files]

    # Get list of config files.
    if config in ['overwrite', 'o']:
        cprint(
            'Overwriting configuration files on target',
            'blue',
            attrs=['bold'])
        call(['rsync', '-avzPLR', '--checksum', '-e ssh'] + config_files +
             [target_dir])

    if config in ['update', 'u']:
        cprint(
            'Adding new configuration files to target', 'blue', attrs=['bold'])
        call(['rsync', '-avzuPLR', '--checksum', '-e ssh'] + config_files +
             [target_dir])

    if not config or config in ['new', 'n']:
        cprint(
            'Adding new configuration files to the target',
            'blue',
            attrs=['bold'])
        call(['rsync', '-avzPLR', '--checksum', '--ignore-existing', '-e ssh']
    ...
    call(['rsync', '-avzPLR', '--checksum', '-e ssh'] + ['version.txt'] +
         [target_dir])
