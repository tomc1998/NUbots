...
def register(command):
    ...
    nbs_tools.extract_images.register(subcommands.add_parser("extract_images_MoCap"))


def run(nbs_command, **kwargs):
    {"stats": nbs_tools.stats.run, "json": nbs_tools.json.run, "extract_images": nbs_tools.extract_images.run, "extract_images_MoCap": nbs_tools.extract_images_MoCap.run}[
        nbs_command
    ](**kwargs)
