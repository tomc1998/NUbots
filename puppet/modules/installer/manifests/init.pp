      ...
      default: {
        ...
        exec { "boost_${arch}_${name}":
          ...
          command     => "${prebuild_cmd} &&
                          ...
                          ./bootstrap.sh --prefix=\"${prefix}/${arch}\" --with-python=\"${prefix}/bin/python3\"  --with-python-root=\"${prefix}\" --with-python-version=3.6 2>&1 | tee bootstrap.log &&
                          ./bjam include=\"${prefix}/${arch}/include\" include=\"${prefix}/include/python3.6m\" library-path=\"${prefix}/${arch}/lib\" ${args_str} -j\$(nproc) -q -a \\
                                cflags=\"${flags}\" cxxflags=\"${flags}\" linkflags=\"${linkflags}\" variant=release threading=multi --layout=system install 2>&1 | tee install.log
                          ${postbuild_cmd}",
          cwd         => "${prefix}/${arch}/src/${name}/${src_dir}",
          environment => $environment,
          path        =>  [ "${prefix}/${arch}/bin", "${prefix}/bin",
